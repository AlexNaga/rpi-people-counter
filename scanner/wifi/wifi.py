...
class Wifi:
    ...
    def discover_devices(self, adapter, scantime):
        verbose = False
        dictionary = "oui.txt"
        number = False
        nearby = False
        ...
        return self.scan(adapter, scantime, verbose, dictionary, number,
                         nearby, jsonprint, out, allmacaddresses, manufacturers, sort, targetmacs)

    def scan(self, adapter, scantime, verbose, dictionary, number, nearby, jsonprint, out, allmacaddresses, manufacturers, sort, targetmacs):
        ...
        if not found_macs:
            print("Found no signals. Make sure %s supports monitor mode." % adapter)
            no_devices_found = 0
        ...
        for mac in found_macs:
            ...
            if allmacaddresses or oui_id in cellphone:
                if not nearby or (nearby and found_macs[mac] > -70):
        ...
        if out:
            with open(out, "a") as f:
                data_dump = {"cellphones": cellphone_people,
                             "time": time.time()}
